...
def test_lookup_major_item_standard_name() -> None:
    ...
    assert result["major_item_standard_name"] == "血脂七项"
...
def test_lookup_major_item_with_fullwidth_brackets() -> None:
    ...
    assert result is not None
    assert "人乳头瘤病毒" in result["major_item_standard_name"]
...
def test_lookup_major_item_real_sample_catalogs() -> None:
    ...
    assert normalizer.lookup("H-凝血功能五项检查")["major_item_standard_name"] == "凝血功能五项检查"
    assert normalizer.lookup("H-女性激素八项")["major_item_standard_name"] == "女性激素八项"
    assert normalizer.lookup("H-女性肿瘤标志物全项")["major_item_standard_name"] == "女性肿瘤标志物全项"
    ...
    assert normalizer.lookup("H-甲功五项TPO")["major_item_standard_name"] == "甲功五项TPO"
